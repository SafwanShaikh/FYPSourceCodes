...
listFirstSemesterITC_GPA = []
j = 0               # For 1st course of first semester j=0, if we train it for second course update j=0 to j=1, for third course j=2 and upto j=4 for fifth course of second semester.
for i in range(int(len(firstSemesterGPAs)/5)):
    listFirstSemesterITC_GPA.append(firstSemesterGPAs[j])
    j = j+5
firstSemesterITC_GPA = np.array(listFirstSemesterITC_GPA)
...
firstSemesterITC_GPA = firstSemesterITC_GPA.reshape(-1, 1)
from sklearn.naive_bayes import GaussianNB
clf = GaussianNB()
X_trainCP = np.array(firstSemesterITC_GPA[0:140])
Y_trainCP = np.array(secondSemesterCP_GPA[0:140])
clf.fit(X_trainCP, Y_trainCP)
X_testCP = np.array(firstSemesterITC_GPA[-37:])
